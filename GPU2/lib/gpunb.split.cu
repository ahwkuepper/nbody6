...
struct Jparticle{
	...
	float  pad;
	Jparticle() {}
	Jparticle(const double mj, const double xj[3], const double vj[3]){
		pos.x = xj[0];
		pos.y = xj[1];
		pos.z = xj[2];
		mass  = mj;
...
struct Iparticle{
	...
	float  dtr;
	Iparticle() {}
	Iparticle(const double h2i, const double dtri, const double xi[3], const double vi[3]){
		pos.x = xi[0];
		pos.y = xi[1];
		pos.z = xi[2];
		h2    = h2i;
...
static void GPUNB_open(int nbmax){
...
#pragma omp parallel
	{
		...
		if(tid < numGPU){
			...
			// gravity kernel
			const int niblock = 1 + (ni-1) / NTHREAD;
			dim3 grid(niblock, NJBLOCK, 1);
			dim3 threads(NTHREAD, 1, 1);
			const int nj = joff[tid+1] - joff[tid];
			gravity_kernel <<< grid, threads >>> 
				(nj, ipbuf[tid], jpbuf[tid], fpart[tid], nbpart[tid]);
		}
	} // end omp parallel
...
extern "C" {
	...
	void gpunb_regf_(
		...
		GPUNB_regf_1st(*ni, h2, dtr, xi, vi);
		GPUNB_regf_2nd(*ni, acc, jrk, pot, *lmax, *nbmax, list);
	}
	void gpunb_regf_1st_(
			int *ni,
			double h2[],
			double dtr[],
			double xi[][3],
